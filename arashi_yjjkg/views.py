...
def index(request):
    ...
    if request.method == "POST":
        ...
        # 「樹木希林」専用読みを一時追加
        is_special_case = (yjjkg == "樹木希林")
        if is_special_case:
            original_readings = kanjiconv.custom_readings["compound"].copy()
            kanjiconv.custom_readings["compound"].update({"樹木": "きき"})
...
'''
            ...
            # 特別読みを復元
            if is_special_case:
                kanjiconv.custom_readings["compound"] = original_readings
